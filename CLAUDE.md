...
# Build TF-IDF index from one or more datasets
python -m src.pipelines.build_index \
  --quality data/raw/quality.jsonl \
  --nq data/raw/nq.jsonl \
  --hotpot data/raw/hotpot.json \
  --split train
...
Three loaders (`quality_loader`, `nq_loader`, `hotpot_loader`) each parse a different format and emit `NormalizedQARecord` objects defined in `schema.py`. This is the only format used downstream—loaders are the only place format-specific logic lives.
...
`evaluate_result()` currently only records query character length, retrieval count, and whether the answer is non-empty. `run_eval.py` invokes the graph per query and writes JSONL output.
...
- **Evaluation metrics are minimal.** `metrics.py` has no F1, BLEU, or exact-match scoring. The `expected_answer` field written to eval output is not used in scoring.
